...
async def get_account_hash(reader: asyncio.streams.StreamReader,
...
async def do_registartions(nick_name: str,
                           host: str, port: str) -> None:

    async with open_connection(server=host, port=port, attempts=3
                               ) as rw:
        reader, writer = rw
        account_hash: str = await get_account_hash(reader, writer, nick_name)
...
def get_credentials(nick_name: str, label: tk.Label,
    ...
    answer = tk.messagebox.askyesno(
        title: str="Warning",
        message: str=f"Is '{nick_name}' nick name correct?")
    if answer:
        asyncio.run(do_registartions(
            nick_name=nick_name, host=host, port=port))
